...
import { FloatingIconsHero, type FloatingIconsHeroProps } from '@/src/components/ui/floating-icons-hero-section';

// --- Original Stylized Company Logo SVG Components from User Request ---
...
export default function App() {
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-white selection:text-black overflow-hidden relative">
      ...
      <nav className="fixed top-0 w-full z-50 p-6 flex justify-between items-center border-b border-white/5 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center">
            <Zap className="text-black w-5 h-5 fill-current" />
          </div>
          <span className="font-bold tracking-tighter text-xl">AUTO-MAGIC</span>
        </div>
        ...
        <button className="px-5 py-2 bg-white text-black rounded-full text-sm font-bold hover:scale-105 transition-transform">
          HIRE ME
        </button>
      </nav>

      {/* Hero Content with Floating Icons */}
      <FloatingIconsHero
        title={
            <div className="max-w-4xl z-10 text-center">
                <motion.div
                    ...
                    className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/5 text-xs font-mono mb-8 mx-auto"
                >
                    <Sparkles className="w-3 h-3 text-yellow-400" />
                    <span>ROBOTS ARE READY TO WORK FOR YOU</span>
                </motion.div>
                ...
                >
                    I MAKE <br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-white/40 italic">BORING STUFF</span> <br />
                    DISAPPEAR.
                </motion.h1>
            </div>
        }
        subtitle="Stop clicking buttons. Start doing big things. I build smart robots using n8n and Claude that do your work while you sleep."
        ctaText="FIX MY BUSINESS"
        ctaHref="#"
        icons={demoIcons}
        className="bg-black"
      />
      ...
      {/* Footer Micro-detail */}
      <footer className="absolute bottom-6 w-full px-12 flex justify-between items-center text-[10px] font-mono opacity-30 uppercase tracking-[0.2em] pointer-events-none">
        <span>Built with Claude Code & Magic</span>
        <span>© 2024 Automation Lab</span>
        <span>Scroll to see robots in action</span>
      </footer>
    </div>
  );
}
